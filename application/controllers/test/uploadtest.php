...
//--------------------------------------------------------------------Leo's code to modify this library--------------------------------------------------
		$dataArray=array();
		$dataArray["photo_file_name_hashed"] = $name;
	   	$dataArray["ip_uploaded"] = $_SERVER['REMOTE_ADDR'];
	   	$dataArray["timestamp_uploaded"] = date("Y-m-d H:i:s");
	   	$dataArray["photo_file_path"] = $upload_dir;
		$imageId = $this -> fms_user_profile_model -> photoFilesInsert($dataArray);
		if ($imageId){
			$this -> fms_user_profile_model -> addProfilePhoto($this -> get_user_id(),$imageId,date("Y-m-d H:i:s"));
		}
//--------------------------------------------------------------------Leo's code to modify this library--------------------------------------------------
        return $file;
    }

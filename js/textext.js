 ...
 *
 * @version 1.3.1
 * @copyright Copyright (C) 2011 Alex Gorbatchev. All rights reserved. Modified Extensively by Waylan Wong, Will Rain IT, Inc (C)2013
 * @license MIT License
 */
(function($, undefined)
{
	...
	{
		// Waylan Wong :: customized for fuzzy search, instead of exact match at start of string.
	...
	p.init = function(input, opts)
	{
		...
		// Waylan :: Flat UI sometimes has a <button> right after the input and having 
		// any element in between them breaks the CSS. If this is the case, we need to
		// disable the submit action of the button as well.
		if (input.next().attr('type') === 'submit' || input.next().hasClass('btn')) {
			hiddenInput.insertAfter(input.next());
			input.next().bind('click', function(event) {
	...
	var cssInjected = true; // waylanwong :: modified to stop injection of CSS, CSS moved to external stylesheet 
...
(function($)
{
	...
	p.init = function(core)
	{
		...
		if(self.opts(OPT_ENABLED) === true)
		{
			...
			// Waylan :: Flat UI will sometimes have a <button> after the input and
			// adding anything in between them will break the CSS.			
			if (input.next().attr('type') === 'submit' || input.next().hasClass('btn')) {
	...
	p.positionDropdown = function()
	{
		...
		// Waylan : SlimScroll breaks up the sizing of the suggestions box
		// so we have to activate SlimScroll here, and then adjust the sizing.
		// Vertical sizing is fixed because SlimScroll requires the height.		
		try {
			container.slimScroll({
				height : parseInt(self.opts(OPT_MAX_HEIGHT).substring(0, self.opts(OPT_MAX_HEIGHT).length-2))+1+'px',
				size : '15px'});
			container.css({'width': '100%'});
		} catch (e) {
			// do not try to use slimScroll because the JS file does not exist
			console.log("In textext.js, missing the slimScroll.js file! Cannot set custom suggestion box size.")
		}	
	...
	var p = TextExtTags.prototype,
		...
		DEFAULT_OPTS = {
			...
			html : {
				...
				tag  : '<div class="text-tag"><div class="text-button"><span class="text-label"/><a title="Remove" class="text-remove"/></div></div>'
			}
		}
		;
...
.text-core .text-wrap .text-tags .text-tag, #create-page #project-desc .text-tag{\
  float: left;\
}\
.text-core .text-wrap .text-tags .text-tag .text-button, #create-page #project-desc .text-button {\
...
.text-core .text-wrap .text-tags .text-tag .text-button a.text-remove, #create-page #project-desc a.text-remove {\
  position: absolute;\
  right: 4px;\
  top: 4px;\
  display: block;\
  width: 13px;\
  height: 13px;\
  background: url("img/tag_close_icon.png") no-repeat;\
  background-size: 12px 12px;\
}\
.text-core .text-wrap .text-tags .text-tag .text-button a.text-remove:hover, #create-page #project-desc a.text-remove:hover {\
  opacity: 0.6;\
}\
.text-core .text-wrap .text-tags .text-tag .text-button a.text-remove:active, #create-page #project-desc a.text-remove:active {\
